...
the passband (it lowers the main lobe only if the passband is narrow enough to cut into it).
  ...
  • --transition <MHz>          skirt steepness — the transition width beyond the passband
                                edge (live).
All three are LIVE: changing one rebuilds the (circularly-)filtered loop into a new /dev/shm
file and swaps it in (blocks.file_source.open) — one brief seam at the swap, then it loops
clean; the flowgraph never stops. Disabling swaps back to the unfiltered loop.
