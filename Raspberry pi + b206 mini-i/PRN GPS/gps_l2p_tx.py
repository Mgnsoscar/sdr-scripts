#!/usr/bin/env python3
"""
GPS L2 P-code transmitter — the PUBLIC (unencrypted) precision code, streamed in real
...
CLI
───
    gps_l2p_tx.py --prn 5 --power -30            # absolute dBm (calibrated), L2 P-code
    gps_l2p_tx.py --prn 5 --gain 60             # relative: raw SDR gain
    gps_l2p_tx.py --self-test                   # validate the generator (no hardware)
    gps_l2p_tx.py --dry-run --prn 5 --gain 60   # exercise the streaming path, no radio
...
# ── signal constants (fixed — this IS GPS L2 P-code) ────────────────────────────────
SIGNAL_NAME = "GPS L2 P"
CARRIER_HZ = 1227.60e6                        # L2
...
def power_map() -> PowerMap:
    """Active power map: the unit's injected L2 P-code calibration if present, else an
        ...
        .number("-Power", "--power", unit="dBm",
                **power_map().power_field_kwargs(), required=False, live=True,
                help="ABSOLUTE power at the delivered plane (dBm). Bounds track the unit's "
                     "L2 P-code calibration when present. Ignored if --gain is given. Live.")
...
def main() -> int:
    ...
    print(f"  carrier        : {CARRIER_HZ/1e6:.3f} MHz (L2)")
