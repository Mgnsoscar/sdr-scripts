#!/usr/bin/env python3
"""
GPS C/A (1.023 Mcps) transmitter for GNU Radio + UHD (Ettus B200-mini family).

Transmit a BPSK GPS C/A Gold code (1.023 Mcps) on the L1 (1575.42 MHz, default) or L2
(1227.6 MHz) carrier — pick the band with --freq — prebuilt once and looped so a Raspberry
Pi sustains the rate with no runtime IQ math.
...
stops. The loop is streamed from RAM by a C++
...
CLI
───
    gps_ca_code_1.023Mcps.py --prn 5 --power -30           # calibrated dBm (main lobe + 2 sidelobes)
    gps_ca_code_1.023Mcps.py --prn 5 --gain 60 --sidelobes 5   # relative gain, main + 5 sidelobes
    gps_ca_code_1.023Mcps.py --prn 5 --freq 1227.6         # same signal on the L2 carrier
    gps_ca_code_1.023Mcps.py --self-test        # verify the Gold-code generator (+ filter/laws, if numpy)
    gps_ca_code_1.023Mcps.py --describe-params  # paramkit JSON schema for the GUI
"""
...
CAL_SIGNAL_ID = "GPS C/A (1.023 Mcps)"
...
SIGNAL_NAME = "GPS C/A (1.023 Mcps)"
...
# Carrier presets: the same 1.023 Mcps C/A signal on either GPS band. Default is L1.
