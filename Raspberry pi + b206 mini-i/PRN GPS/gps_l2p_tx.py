#!/usr/bin/env python3
"""
GPS L2 P-code transmitter for GNU Radio + UHD (Ettus B200-mini family).

Purpose
───────
Transmit a wideband (~20 MHz) BPSK PRN at the L2 carrier (1227.60 MHz) as a
stand-in for the GPS L2 P-code, on a Raspberry Pi.

⚠  PLACEHOLDER CODE SOURCE
   This script does NOT yet generate the real GPS P-code. Real P-code is a
   different sequence entirely — four 12-stage LFSRs (X1A/X1B/X2A/X2B per ICD-200)
   clocked at 10.23 Mcps with a per-satellite X2 chip delay and a one-week period.
   Until that generator is written, this script emits the 1023-chip C/A Gold code
   clocked at 10.23 Mcps (the old gps_prn "10 MHz code"): it reproduces the P-code's
   ~20 MHz spectral WIDTH and BPSK envelope, but NOT its actual chip sequence — so
   it will not correlate as P-code. Swap ca_code()/build_iq_buffer() for a true
   P-code generator when ready; the RF/power plumbing stays the same.

This is one of three scripts split out from the old combined gps_prn_tx.py:
    gps_l1ca_tx.py   — GPS L1 C/A          (1.023 Mcps @ L1)
    gps_l1p_tx.py    — GPS L1 P-code       (10.23 Mcps @ L1)
    gps_l2p_tx.py    — GPS L2 P-code       (this file, 10.23 Mcps @ L2)
...
2. sc8 OVER THE WIRE. cpu_format=fc32 but otw_format=sc8 halves the USB payload —
   needed for the ~40 MS/s a wideband code wants. A PRN is constant-modulus BPSK,
   so 8-bit I/Q costs nothing that matters here.
...
CLI
───
    gps_l2p_tx.py --prn 5 --power -30 --samp_rate 40.92
    gps_l2p_tx.py --self-test        # verify the Gold-code generator, no hardware
    gps_l2p_tx.py --describe-params  # paramkit JSON schema for the GUI
"""
...
# ── Signal constants (fixed for this script — it IS GPS L2 P-code) ──────────────

CARRIER_HZ = 1227.60e6         # GPS L2
CODE_RATE_MCPS = 10.23         # P-code chip rate (~20 MHz null-to-null)
SIGNAL_NAME = "GPS L2 P-code"
# See the PLACEHOLDER note in the module docstring: the chip SEQUENCE is still the
# C/A Gold code (clocked ×10), not the real P-code, until a P-code generator lands.
PLACEHOLDER = True

CODE_LEN = 1023                # chips in the (placeholder) C/A Gold code period

SAMPLE_RATES = {
    "20.46 MHz (Minimum -> Main lobe only)": 20.46,
    "40.92 MHz (Default -> Main lobe + skirts)": 40.92,
    "61.38 MHz (Maximum -> Widest skirts)": 61.38,
}
DEFAULT_SAMPLE_RATE = 40.92
...
# Used only by --self-test to prove the (placeholder) generator matches the standard.
_FIRST10_OCTAL = {
...
# ── Code generation (PLACEHOLDER: C/A Gold code, to be replaced by real P-code) ──

def ca_code(prn: int) -> list[int]:
    """Return the 1023-chip GPS C/A Gold code for a PRN (1..32) as a list of 0/1.

    ⚠ PLACEHOLDER for P-code (see the module docstring). Two 10-stage LFSRs (both
    seeded all-ones):
      G1: x^10 + x^3 + 1
...
def _self_test() -> int:
    ...
    success (usable as a process exit code); prints a per-PRN report. (Validates the
    placeholder generator; a real P-code generator will need its own self-test.)"""
    print("NOTE: P-code source is a PLACEHOLDER (C/A Gold code ×10) — see docstring.")
    ok = True
...
def _build_top_block(code_file: str, center_freq_hz: float, samp_rate_hz: float,
                     gain_db: float, amplitude: float, otw_format: str, extra_args: str):
...
def build_script() -> Script:
    s = (
        Script(
            f"{SIGNAL_NAME} transmitter (PLACEHOLDER code: C/A Gold code ×10, not "
            "the real P-code — see the script header). Level is set in dBm; edit the "
            "USER CALIBRATION block at the top of the script to match your rig."
        )
        .integer(
            "-PRN", "--prn",
            min=1, max=32,
            default=1,
            required=True,
            help="GPS satellite PRN / (placeholder) Gold code index (1 to 32). "
                 "Fixed per run."
        )
        .number(
            "-Power", "--power",
            unit="dBm",
            **power_map().power_field_kwargs(),
...
def main() -> int:
    ...
    tmpdir = tempfile.mkdtemp(prefix="gps_l2p_", dir=shm)
    ...
    if PLACEHOLDER:
        print("  ⚠ PLACEHOLDER code source: C/A Gold code ×10, NOT real P-code")
