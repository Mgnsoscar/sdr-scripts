#!/usr/bin/env python3
"""
GPS L1 P-code transmitter for GNU Radio + UHD (Ettus B200-mini family).

Purpose
───────
Transmit a wideband (~20 MHz) BPSK PRN at the L1 carrier (1575.42 MHz) as a
stand-in for the GPS L1 P-code, on a Raspberry Pi.
...
This is one of three scripts split out from the old combined gps_prn_tx.py:
    ...
    gps_l1p_tx.py    — GPS L1 P-code       (this file, 10.23 Mcps @ L1)
...
CLI
───
    gps_l1p_tx.py --prn 5 --power -30 --samp_rate 40.92
    gps_l1p_tx.py --self-test        # verify the Gold-code generator, no hardware
...
def main() -> int:
    ...
    tmpdir = tempfile.mkdtemp(prefix="gps_l1p_", dir=shm)
